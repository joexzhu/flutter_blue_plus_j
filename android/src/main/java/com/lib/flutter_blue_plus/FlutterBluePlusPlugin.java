...
public class FlutterBluePlusPlugin implements
            ...
            switch (call.method) {
                ...
                case "startScan": {
                    ...
                    ensurePermissions(permissions, (granted, perm) -> {
                        ...
                        if (scanner == null) {
                            result.error("startScan",
                                    String.format("getBluetoothLeScanner() is null. Is the Adapter on?"), null);
                            return;
                        }
    ...
    private ChrFound locateCharacteristic(BluetoothGatt gatt,
        ...
        // characteristic
        BluetoothGattCharacteristic characteristic = getCharacteristicFromArray(characteristicId,
        ...
        if (scanCallback2 == null) {
            scanCallback2 = new BluetoothAdapter.LeScanCallback() {
                ...
                public void onLeScan(BluetoothDevice device, int i, byte[] bytes) {
                    // see BmScanResponse
                    HashMap<String, Object> response = new HashMap<>();
                    response.put("advertisements", Arrays.asList(bmScanAdvertisement2(device, bytes)));
    ...
    HashMap<String, Object> bmScanAdvertisement2(BluetoothDevice device, byte[] bytes) {
        ...
        for (AdRecord record : records) {
            if ((record.type & 0xFF) == 0xFF && record.data.length > 3) {
                HashMap<Integer, String> manufDataB = new HashMap<Integer, String>();
                manufDataB.put((record.data[0] & 0xFF) | (record.data[1] << 8),
                        bytesToHex(Arrays.copyOfRange(record.data, 2, record.data.length)));
    ...
    HashMap<String, Object> bmBluetoothService(BluetoothDevice device, BluetoothGattService service,
            BluetoothGatt gatt) {
    ...
    public void printScanRecord(byte[] scanRecord) {
        ...
        try {
            ...
            Log.d("DEBUG", "decoded String : " + bytesToHex(scanRecord));
